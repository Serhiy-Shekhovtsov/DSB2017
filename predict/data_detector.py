...
class DataBowl3Detector(Dataset):
    def __init__(self, dirlist, phase='test', split_comber=None, **config):
    ...
    def __getitem__(self, idx, split=None):
        ...
        if self.phase != 'test':
            ...
            imgs = np.pad(
                imgs, [[0, 0], [0, pz - nz], [0, ph - nh], [0, pw - nw]],
            ...
            coord2, nzhw2 = self.split_comber.split(
                coord, side_len=side_len, max_stride=max_stride, margin=margin)

            assert np.all(nzhw == nzhw2)
            imgs = (imgs.astype(np.float32) - 128) / 128

            return (
                torch.from_numpy(imgs.astype(np.float32)),
                bboxes,
                torch.from_numpy(coord2.astype(np.float32)),
                np.array(nzhw))
...
def augment(sample, target, bboxes, coord, ifflip=True, ifrotate=True, ifswap=True):
    if ifrotate:
        ...
        while not validrot:
            ...
            if (
                np.all(newtarget[:3] > target[3]) and
                np.all(newtarget[:3] < np.array(sample.shape[1:4]) - newtarget[3])
            ):
                validrot = True
                target = newtarget
...
class Crop(object):
    ...
    def __call__(self, imgs, target, bboxes, isScale=False, isRand=False):
        ...
        xx, yy, zz = np.meshgrid(
            np.linspace(
                normstart[0], normstart[0] + normsize[0],
                self.crop_size[0] // self.stride
            ),
            np.linspace(
                normstart[1], normstart[1] + normsize[1],
                self.crop_size[1] // self.stride
            ),
            np.linspace(
                normstart[2], normstart[2] + normsize[2],
