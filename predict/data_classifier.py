...
class DataBowl3Classifier(Dataset):
    def __init__(self, dirlist, phase='test', **config):
    ...
    def __getitem__(self, idx, split=None):
        ...
        croplist = np.zeros(
            [topk, 1, self.crop_size[0], self.crop_size[1], self.crop_size[2]]
        ).astype('float32')

        coordlist = np.zeros(
            [
                topk, 3, self.crop_size[0] // self.stride,
                self.crop_size[1] // self.stride, self.crop_size[2] // self.stride
            ]).astype('float32')
        ...
        if self.phase != 'test':
            ...
            return (
                torch.from_numpy(croplist).float(),
                torch.from_numpy(coordlist).float(),
                torch.from_numpy(isnodlist).int(),
                torch.from_numpy(y))
        else:
...
class simpleCrop():
    ...
    def __call__(self, imgs, target):
        ...
        crop = imgs[
            :, start[0]:start[0] + crop_size[0], start[1]:start[1] + crop_size[1],
