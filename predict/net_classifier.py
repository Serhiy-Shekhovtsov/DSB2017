...
class Net(nn.Module):
    def __init__(self):
        ...
        for i in range(len(num_blocks_back)):
            ...
            for j in range(num_blocks_back[i]):
                if j == 0:
                    ...
                    back = self.featureNum_back[i + 1]
                    forw = self.featureNum_forw[i + 2]

                    blocks.append(
                        PostRes(back + forw + extra, self.featureNum_back[i]))
                else:
                    blocks.append(
        ...
        self.output = nn.Sequential(
            ...
            nn.ReLU(), nn.Conv3d(64, 5 * len(config['anchors']), kernel_size=1))
...
class Classifier(nn.Module):
    ...
    def forward(self, xlist, coordlist):
        ...
        centerFeat = self.pool(
            noduleFeat[
                :, :, featshape[2] // 2 - 1:featshape[2] // 2 + 1,
