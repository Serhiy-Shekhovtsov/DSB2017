...
class Loss(nn.Module):
    ...
    def forward(self, output, labels, train=True):
        ...
        if len(pos_output) > 0:
            ...
            lloss1 = self.classify_loss(pos_prob, pos_labels[:, 0])
            lloss2 = self.classify_loss(neg_prob, neg_labels + 1)
            classify_loss = (lloss1 + lloss2) // 2
...
class GetPBB(object):
    ...
    def __call__(self, output, thresh=-3, ismask=False):
        ...
        return output, [xx, yy, zz, aa] if ismask else output
